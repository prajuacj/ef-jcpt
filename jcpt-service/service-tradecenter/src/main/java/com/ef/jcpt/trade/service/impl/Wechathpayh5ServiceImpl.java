...
import com.ef.jcpt.trade.config.CommonConfig;
import com.ef.jcpt.trade.config.WechatpayConfig;
...
import com.ef.jcpt.trade.util.pay.WeixinUtil;
...
public class Wechathpayh5ServiceImpl implements IWechatpayh5Service {
	...
	@Override
	public Map<String, Object> submitwhchat(String orderId) {
		// TODO Auto-generated method stub
		Map<String, Object> map = new HashMap();
		try {
			// 应用ID
			String appid = WechatpayConfig.appid;
			String backUri = CommonConfig.payserviceurl + WechatpayConfig.backUri;

			// 授权后要跳转的链接所需的参数一般有会员号，金额，订单号之类，
			// 最好自己带上一个加密字符串将金额加上一个自定义的key用MD5签名或者自己写的签名,
			// 比如 Sign = %3D%2F%CS%
			PayInfoBo orderPay = orderPayServiceImpl.queryPayInfoByOrderId(orderId);
			if (orderPay != null) {
				// String order=new String("订单号:".getBytes(),"UTF-8");
				String order = "订单号:";
				backUri = backUri + "?userId=" + "" + "&orderId=" + orderPay.getOrderId() + "&goodstype=" + "7"
						+ "&goodsname=" + order + orderPay.getOrderId() + "&supportvalue=" + orderPay.getPayAmount();
				// URLEncoder.encode 后可以在backUri 的url里面获取传递的所有参数
				backUri = URLEncoder.encode(backUri, "UTF-8");
				// scope 参数视各自需求而定，这里用scope=snsapi_base
				// 不弹出授权页面直接授权目的只获取统一支付接口的openid
				String url = WechatpayConfig.oauth2 + "appid=" + appid + "&redirect_uri=" + backUri
	...
	public void toPay2(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		if (isweixin) {
			...
			// 微信支付商户号
			String partner = WechatpayConfig.partner;
			// API密钥，在商户平台设置
			String apikey = WechatpayConfig.apikey;
			...
			String openId = "";
			// 获取access_token跳转
			String URL = "https://api.weixin.qq.com/sns/oauth2/access_token?token=111&appid=" + appid + "&secret="
					+ appsecret + "&code=" + code + "&grant_type=authorization_code";
			HttpRespons temp = HttpConnect.getInstance().doGetStr(URL);
			String tempValue = "";
			if (temp == null) {
				// response.sendRedirect("/weChatpay/error.jsp");
				response.sendRedirect(CommonConfig.payserviceurl + "/cart/payerror");
				return;
			} else {
				try {
			...
			String notify_url = CommonConfig.payserviceurl + WechatpayConfig.updateorderstate;
			...
			String xml = "<xml>" + "<appid>" + appid + "</appid>" + "<mch_id>" + mch_id + "</mch_id>" + "<nonce_str>"
					+ nonce_str + "</nonce_str>" + "<sign>" + sign + "</sign>" + "<body><![CDATA[" + goodsname
					+ "]]></body>" + "<attach>" + userId + "</attach>" + "<out_trade_no>" + orderId + "</out_trade_no>"
					+
					// 金额，这里写的1 分到时修改
			...
			String url = ComContents.payserviceurl + "/weChat/pay.html?appId=" + appid2 + "&timeStamp=" + timestamp2
					+ "&nonceStr=" + nonceStr2 + "&packageValue=" + packages + "&paySign=" + finalsign;
			// LogUtil.info(url);
			response.sendRedirect(url);
			...
			response.getWriter()
					.write("<script language='javascript'>alert('您的微信不是5.0以上版本，无法使用微信支付！');history.go(-1);</script>");
		}
	}
	...
	public SNSUserInfo getOauth2AccessToken(String Appid, String appSecret, String code) {
		...
		String requestUrl = WachatContent.getaccesstoken + "appid=" + Appid + "&secret=" + appSecret + "&code=" + code
	...
	public SNSUserInfo refreshOauth2AccessToken(String Appid, String refreshToken) {
		...
		WeiXinOauth2Token wat = null;
		SNSUserINfo snsUserINfo = null;
		String requestUrl = WachatContent.refreshtoken + "appid=" + Appid + "&grant_type=refresh_token&refresh_token="
				+ refreshToken;
		// 获取网页授权凭证
		JSONObject jsonobject = WeixinUtil.httpRequest(requestUrl, "GET", null);
		if (jsonobject != null) {
	...
	public SNSUserInfo getSNSUSerInfo(String accessToken, String openid) {
		...
		SNSUserINfo snsUserInfo = null;
		// 拼接链接地址
		String requestUrl = WachatContent.getuserifo + "access_token=" + accessToken + "&openid=" + openid
				+ "&lang=zh_CN";
		JSONObject jsonobject = WeixinUtil.httpRequest(requestUrl, "GET", null);
		if (jsonobject != null) {
			try {
	...
	public Map<String, String> unifiedOrder(String paySn, String os, String openCode, String openId, String spbillIp) {
		...
		try {
			...
			if (pay != null) {
				paymentTallyService
						.savePaymentTally(
								os.equals(WachatContent.PAYMENT_OS_APP) ? "mp_wechatpay"
										: os.equals(WachatContent.PAYMENT_TRADE_TYPE_MWEB) ? "h5_wechat"
												: "public_wechat",
								"微信支付", pay,
								os.equals(WachatContent.PAYMENT_OS_APP) ? PaymentTallyState.PAYMENTTALLY_TREM_MB
										: PaymentTallyState.PAYMENTTALLY_TREM_H5);
				// String order=new String("订单号:".getBytes(),"UTF-8");
				String orderId = pay.getPaySn();
				String notifyurl = WachatContent.getPropertyValue(WachatContent.WXPAY_NOTIFYURL_PREFIX
						+ (os.equals(WachatContent.PAYMENT_TRADE_TYPE_MWEB) ? WachatContent.PAYMENT_OS_WBMALL : os));
				BigDecimal payAmount = pay.getPayAmount();
				// 乘100
				BigDecimal v2 = new BigDecimal("100");
				int totalFee = payAmount.multiply(v2).intValue();
				WXMallPayConfig config = new WXMallPayConfig(
						os.equals(WachatContent.PAYMENT_TRADE_TYPE_MWEB) ? WachatContent.PAYMENT_OS_WBMALL : os, false);
				WXPay wxpay = new WXPay(config);

				if (StringUtils.isEmpty(openId) && !os.equals(WachatContent.PAYMENT_OS_APP)
						&& !os.equals(WachatContent.PAYMENT_TRADE_TYPE_MWEB)) {
				...
				data.put("trade_type",
						os.equals(WachatContent.PAYMENT_OS_APP) ? WachatContent.PAYMENT_OS_APP
								: os.equals(WachatContent.PAYMENT_TRADE_TYPE_MWEB)
										? WachatContent.PAYMENT_TRADE_TYPE_MWEB
										: WachatContent.PAYMENT_TRADE_TYPE_WXPAY); // 此处指定微信的支付方式
				if (!os.equals(WachatContent.PAYMENT_OS_APP) && !os.equals(WachatContent.PAYMENT_TRADE_TYPE_MWEB)) {
					data.put("openid", openId);
				} /*
					 * if (tradeType.equals("APP")) { data.put("appid", "wxd6fbba723e1911b1");
				...
				if (os.equals(WachatContent.PAYMENT_TRADE_TYPE_MWEB)) { // 微信的H5支付时参数
					...
					jsonDetail.put("wap_url", this.getWapUrlByOs(WachatContent.PAYMENT_OS_WBMALL)); // 暂时固定为wbmall，可能需要新增参数trade_type
																									// 调用的支付方式如为H5支付时可能会有多种跳转可能
	...
	public String getKeyByOs(String os) {
		// TODO Auto-generated method stub
		return WachatContent.getPropertyValue(WachatContent.WXPAY_KEY_PREFIX
				+ (os.equals(WachatContent.PAYMENT_TRADE_TYPE_MWEB) ? WachatContent.PAYMENT_OS_WBMALL : os));
	}
