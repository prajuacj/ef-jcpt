...
import com.ef.jcpt.trade.config.AlipayConfig;
...
import com.ef.jcpt.trade.util.pay.AlipayNotify;
import com.ef.jcpt.trade.util.pay.AlipaySubmit;

public class Alipayh5ServiceImpl implements IAlipayh5Service {

	private static final Logger log = LoggerFactory.getLogger(Alipayh5ServiceImpl.class);
	...
	public String toPay(Alipayh5Bo bo) {
		...
		// 必填，不能修改
		// 服务器异步通知页面路径
		String notify_url = CommonConfig.payserviceurl + AlipayConfig.alipayh5backUrl;
		// 需http://格式的完整路径，不能加?id=123这类自定义参数
		// 页面跳转同步通知页面路径
		String return_url = CommonConfig.payserviceurl + AlipayConfig.alipayh5frontUrl;
		// 需http://格式的完整路径，不能加?id=123这类自定义参数，不能写成http://localhost/
		// 商户订单号
		// String out_trade_no = pay.getPaySn();
		String out_trade_no = bo.getOutTradeNo();
		// 商户网站订单系统中唯一订单号，必填
		// 订单名称
		String subject = "超级购物";
		// 必填
		// 付款金额
		String total_fee = bo.getPayAmount() + "";
		// 必填
		// 商品展示地址
		// String show_url = ComContents.payserviceurl+"/m/goods/goodsdetail?id=34";

		String show_url = bo.getBackUrl();
